...
async fn run_user_expiration_check(state: &AppState) -> Result<(), String> {
    ...
    if !expired_account_ids.is_empty() {
        let placeholders = expired_account_ids
            .iter()
            .map(|_| "?")
            .collect::<Vec<&str>>()
            .join(",");
    ...
    if !expired_level_ids.is_empty() {
        let placeholders = expired_level_ids
            .iter()
            .map(|_| "?")
            .collect::<Vec<&str>>()
            .join(",");
...
async fn run_daily_tasks(state: &AppState) -> Result<(), String> {
    ...
    let _ = sqlx::query("DELETE FROM node_status")
        .execute(&state.db)
        .await;
...
async fn aggregate_traffic_for_date(
    state: &AppState,
    record_date: &str,
...
async fn aggregate_system_traffic(
    state: &AppState,
    record_date: &str,
) -> Result<SystemTrafficStats, String> {
    let row = sqlx::query(
        r#"
    SELECT
