...
fn parse_client_data(
    ...
    let obj = decoded
        .as_object()
        .ok_or_else(|| "clientDataJSON 无效".to_string())?;
...
fn import_cose_public_key(cose: &CborValue, alg: i64) -> Result<PublicKeyKind, String> {
    ...
    if kty == 2 && resolved_alg == -7 {
        ...
        let key =
            EcdsaVerifyingKey::from_sec1_bytes(&sec1).map_err(|_| "公钥数据不完整".to_string())?;
...
fn decode_cbor(data: &[u8], offset: usize) -> Result<(CborValue, usize), String> {
    ...
    match major {
        ...
        3 => {
            ...
            let text = String::from_utf8(data[cursor..end].to_vec())
                .map_err(|_| "CBOR 文本无效".to_string())?;
