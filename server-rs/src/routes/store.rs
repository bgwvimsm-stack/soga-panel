...
async fn get_package_detail(State(state): State<AppState>, Path(id): Path<i64>) -> Response {
    if id <= 0 {
...
async fn post_purchase(
    ...
    if !pay.success || pay.pay_url.is_none() {
        return error(
            ...
            pay.message.as_deref().unwrap_or("创建支付订单失败"),
            None,
        );
    }

    let is_mixed = stored_purchase_type.starts_with("balance_");
    let status_text = if is_mixed {
        "待支付差额"
    } else {
        "待支付"
    };
...
async fn get_purchase_records(
    ...
    let total_row =
        sqlx::query("SELECT COUNT(*) as total FROM package_purchase_records WHERE user_id = ?")
            .bind(user_id)
            .fetch_optional(&state.db)
            .await;
    let total_row = match total_row {
...
async fn get_package_by_id(
    state: &AppState,
    package_id: i64,
) -> Result<Option<PackageSummary>, String> {
    let row = sqlx::query(
    r#"
    SELECT id, name, CAST(price AS DOUBLE) as price, traffic_quota, validity_days, speed_limit, device_limit,
...
async fn get_coupon_by_code(state: &AppState, code: &str) -> Result<Option<CouponRow>, String> {
    ...
    Ok(Some(CouponRow {
        ...
        start_at: row
            .try_get::<Option<i64>, _>("start_at")
            .unwrap_or(Some(0))
            .unwrap_or(0),
        end_at: row
            .try_get::<Option<i64>, _>("end_at")
            .unwrap_or(Some(0))
            .unwrap_or(0),
...
fn map_package_row(row: &sqlx::mysql::MySqlRow) -> Value {
    let price = parse_decimal(row, "price", 0.0);
    let traffic_quota = row
        .try_get::<Option<i64>, _>("traffic_quota")
        .unwrap_or(Some(0))
        .unwrap_or(0);
    let validity_days = row
