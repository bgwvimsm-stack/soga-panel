...
async fn post_gift_cards(
    ...
    let code_prefix = sanitize_code(
        body.code_prefix
            .unwrap_or_else(|| "GC".to_string())
            .as_str(),
    );
    let code_prefix = if code_prefix.is_empty() {
        "GC".to_string()
    } else {
        code_prefix
    };
  ...
  .await;
    ...
    for _ in 0..quantity {
      ...
      "#,
        ...
        .bind(duration_days)
        .bind(traffic_value_gb)
        .bind(if card_type == "reset_traffic" {
            None::<i64>
        } else {
            reset_traffic_gb
        })
...
async fn put_gift_card(
    ...
    let allowed_fields = [
        ...
        (
            "balance_amount",
            body.balance_amount.map(|value| json!(value)),
        ),
        (
            "duration_days",
            body.duration_days.map(|value| json!(value)),
        ),
        (
            "traffic_value_gb",
            body.traffic_value_gb.map(|value| json!(value)),
        ),
        (
            "reset_traffic_gb",
            body.reset_traffic_gb.map(|value| json!(value)),
        ),
        ("package_id", body.package_id.map(|value| json!(value))),
        ("max_usage", body.max_usage.map(|value| json!(value))),
        (
            "per_user_limit",
            body.per_user_limit.map(|value| json!(value)),
        ),
        (
            "start_at",
            body.start_at
    ...
    success(
        json!({ "id": card_id, "message": "礼品卡已更新" }),
        "礼品卡已更新",
    )
    .into_response()
}
...
async fn get_redemptions(
    ...
    let total_row =
        sqlx::query("SELECT COUNT(*) as total FROM gift_card_redemptions WHERE card_id = ?")
            .bind(card_id)
            .fetch_optional(&state.db)
            .await;
