...
pub fn router() -> Router<AppState> {
  Router::new()
    .route("/", get(get_subscription_logs))
    .route("/{id}", delete(delete_subscription_log))
...
async fn get_subscription_logs(
  ...
  let sub_type = query.sub_type.unwrap_or_default().trim().to_string();
  ...
  if !sub_type.is_empty() {
    conditions.push("s.type = ?".to_string());
    params.push(SqlParam::String(sub_type));
  }
  if !ip_search.is_empty() {
    conditions.push("s.request_ip LIKE ?".to_string());
  ...
  let list_sql = format!(
    r#"
    SELECT s.id, s.user_id, s.type, s.request_ip, s.request_time, s.request_user_agent,
  ...
  let logs = rows
    ...
    .map(|row| {
      json!({
        ...
        "request_ip": row.try_get::<Option<String>, _>("request_ip").ok().flatten().unwrap_or_default(),
        "request_time": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("request_time").ok().flatten()),
        "request_user_agent": row.try_get::<Option<String>, _>("request_user_agent").ok().flatten().unwrap_or_default()
      })
    })
    .collect::<Vec<Value>>();
...
async fn delete_subscription_log(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Path(id): Path<i64>
) -> Response {
  ...
  if let Err(err) = sqlx::query("DELETE FROM subscriptions WHERE id = ?")
    .bind(id)
    .execute(&state.db)
    .await
  {
  ...
  success(json!({ "message": "订阅日志删除成功" }), "Success").into_response()
}
...
async fn post_batch_delete(
  ...
  let sql = format!("DELETE FROM subscriptions WHERE id IN ({placeholders})");
  ...
  success(
    json!({ "message": format!("成功删除 {} 条订阅日志", ids.len()), "deleted_count": ids.len() }),
    "Success"
  )
  .into_response()
}
...
async fn post_export_csv(
  ...
  let where_clause = if ids.is_empty() {
    ...
    format!("WHERE s.id IN ({})", ids.iter().map(|_| "?").collect::<Vec<&str>>().join(","))
  };

  let sql = format!(
    r#"
    SELECT u.email as user_email, s.type as subscription_type, s.request_ip as access_ip,
           s.request_user_agent as client_ua, s.request_time as access_time
    FROM subscriptions s
    LEFT JOIN users u ON s.user_id = u.id
    {where_clause}
    ORDER BY s.request_time DESC
  ...
  let headers = ["用户邮箱", "订阅类型", "访问IP", "客户端UA", "访问时间"];
  let mut csv = format!("{}\n", headers.join(","));
  for row in rows {
    let line = [
      escape_csv(row.try_get::<Option<String>, _>("user_email").ok().flatten().unwrap_or_default()),
      escape_csv(row.try_get::<Option<String>, _>("subscription_type").ok().flatten().unwrap_or_default()),
      escape_csv(row.try_get::<Option<String>, _>("access_ip").ok().flatten().unwrap_or_default()),
      escape_csv(row.try_get::<Option<String>, _>("client_ua").ok().flatten().unwrap_or_default()),
      escape_csv(row.try_get::<Option<NaiveDateTime>, _>("access_time").ok().flatten().map(|dt| dt.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or_default())
    ];
    csv.push_str(&format!("{}\n", line.join(",")));
  }

  build_csv_response("subscription_logs.csv", csv)
}
