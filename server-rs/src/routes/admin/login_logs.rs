...
async fn post_export_csv(
    ...
    for row in rows {
        let line = [
            ...
            escape_csv(
                row.try_get::<Option<String>, _>("login_ip")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
            escape_csv(
                row.try_get::<Option<String>, _>("login_ua")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
            escape_csv(
                format_datetime(
                    row.try_get::<Option<NaiveDateTime>, _>("login_time")
                        .ok()
                        .flatten(),
                )
                .unwrap_or_default(),
            ...
            escape_csv(
                row.try_get::<Option<String>, _>("login_status")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
