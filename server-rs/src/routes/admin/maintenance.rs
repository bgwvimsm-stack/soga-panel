...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            post(post_reset_all_subscriptions),
        )
        ...
        .route(
            "/generate-traffic-test-data",
            post(post_generate_traffic_test_data),
        )
}
...
async fn post_reset_all_passwords(
    ...
    for row in users {
        ...
        if sqlx::query(
            "UPDATE users SET uuid = ?, passwd = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        )
        .bind(uuid)
        .bind(passwd)
        .bind(user_id)
...
async fn post_reset_invite_codes(
    ...
    for row in users {
        ...
        if sqlx::query(
            "UPDATE users SET invite_code = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        )
        .bind(invite_code)
        .bind(user_id)
        .execute(&state.db)
...
async fn post_generate_traffic_test_data(
    ...
    for day in 0..days {
        ...
        for idx in 0..nodes_for_day {
            ...
            let upload = ((50.0_f64 + rand::thread_rng().gen_range(0.0_f64..450.0_f64))
                * 1024.0
                * 1024.0
                * base_multiplier)
                .floor() as i64;
            let download = ((200.0_f64 + rand::thread_rng().gen_range(0.0_f64..1800.0_f64))
                * 1024.0
                * 1024.0
                * base_multiplier)
                .floor() as i64;
