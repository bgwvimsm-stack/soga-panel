...
async fn export_batch_csv(
    ...
    for row in rows {
        ...
        let line = [
            escape_csv(
                row.try_get::<Option<String>, _>("code")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
            ...
            escape_csv(
                row.try_get::<Option<String>, _>("card_type")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
            ...
            escape_csv(
                row.try_get::<Option<String>, _>("package_name")
                    .ok()
                    .flatten()
                    .unwrap_or_default(),
            ),
            ...
            escape_csv(
                format_datetime(
                    row.try_get::<Option<NaiveDateTime>, _>("start_at")
                        .ok()
                        .flatten(),
                )
                .unwrap_or_default(),
            ...
            escape_csv(
                format_datetime(
                    row.try_get::<Option<NaiveDateTime>, _>("end_at")
                        .ok()
                        .flatten(),
                )
                .unwrap_or_default(),
...
fn map_batch_row(row: &sqlx::mysql::MySqlRow) -> Value {
    let card_count = row
        .try_get::<Option<i64>, _>("card_count")
        .ok()
        .flatten()
        .unwrap_or(0);
    let used_count = row
