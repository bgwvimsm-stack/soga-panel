...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .nest(
            "/gift-cards",
            gift_cards::router().merge(gift_card_batches::router()),
        )
...
async fn get_system_stats(
    ...
    let disabled_users = users_row
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<i64>, _>("disabled_users")
                .ok()
                .flatten()
        })
        .unwrap_or(0);
    ...
    let today_traffic = traffic_row
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<i64>, _>("today_traffic")
                .ok()
                .flatten()
        })
        .unwrap_or(0);
    let average_quota = traffic_row
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<f64>, _>("average_quota")
                .ok()
                .flatten()
        })
        .unwrap_or(0.0);
...
async fn get_node_status(
    ...
    let enabled_nodes = fetch_count(
        &state,
        "SELECT COUNT(*) as total FROM nodes WHERE status = 1",
    )
    .await;
