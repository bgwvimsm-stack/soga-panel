use axum::extract::{Path, Query, State};
use axum::http::{HeaderMap, StatusCode};
use axum::response::{IntoResponse, Response};
use axum::routing::{delete, get, post, put};
use axum::{Extension, Json, Router};
use chrono::{DateTime, NaiveDate, NaiveDateTime};
use serde::Deserialize;
use serde_json::{json, Value};
use sqlx::Row;

use crate::cache::cache_delete_by_prefix;
use crate::response::{error, success};
use crate::state::AppState;

use super::super::auth::require_admin_user_id;

#[derive(Deserialize)]
struct AuditRulesQuery {
  page: Option<i64>,
  limit: Option<i64>,
  #[serde(rename = "pageSize")]
  page_size: Option<i64>,
  search: Option<String>,
  enabled: Option<String>
}

#[derive(Deserialize)]
struct AuditRuleRequest {
  name: Option<String>,
  description: Option<String>,
  rule: Option<String>,
  enabled: Option<i64>
}

#[derive(Deserialize)]
struct AuditRuleTestRequest {
  content: Option<String>
}

#[derive(Deserialize)]
struct AuditLogsQuery {
  page: Option<i64>,
  limit: Option<i64>,
  #[serde(rename = "pageSize")]
  page_size: Option<i64>,
  #[serde(rename = "user_id")]
  user_search: Option<String>,
  #[serde(rename = "rule_id")]
  rule_id: Option<String>,
  #[serde(rename = "target_ip")]
  target_ip: Option<String>,
  start_date: Option<String>,
  end_date: Option<String>
}

#[derive(Deserialize)]
struct BatchIdsRequest {
  ids: Option<Vec<i64>>
}

pub fn router() -> Router<AppState> {
  Router::new()
    .route("/audit-rules", get(get_audit_rules))
    .route("/audit-rules", post(post_audit_rule))
    .route("/audit-rules/{id}", put(put_audit_rule))
    .route("/audit-rules/{id}", delete(delete_audit_rule))
    .route("/audit-rules/{id}/test", post(post_audit_rule_test))
    .route("/audit-logs", get(get_audit_logs))
    .route("/audit-logs/{id}", delete(delete_audit_log))
    .route("/audit-logs/batch-delete", post(post_audit_log_batch_delete))
}

async fn get_audit_rules(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Query(query): Query<AuditRulesQuery>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }

  let page = query.page.unwrap_or(1).max(1);
  let limit_raw = query.limit.or(query.page_size).unwrap_or(20);
  let limit = limit_raw.max(1).min(200);
  let offset = (page - 1) * limit;

  let search = query.search.unwrap_or_default().trim().to_string();
  let enabled = parse_optional_i64(query.enabled.as_deref());

  let mut conditions: Vec<String> = Vec::new();
  let mut params: Vec<SqlParam> = Vec::new();

  if !search.is_empty() {
    let pattern = format!("%{search}%");
    conditions.push("(name LIKE ? OR description LIKE ?)".to_string());
    params.push(SqlParam::String(pattern.clone()));
    params.push(SqlParam::String(pattern));
  }
  if let Some(value) = enabled {
    conditions.push("enabled = ?".to_string());
    params.push(SqlParam::I64(value));
  }

  let where_clause = if conditions.is_empty() {
    String::new()
  } else {
    format!("WHERE {}", conditions.join(" AND "))
  };

  let total_sql = format!("SELECT COUNT(*) as total FROM audit_rules {where_clause}");
  let mut total_query = sqlx::query(&total_sql);
  total_query = bind_params(total_query, &params);
  let total_row = match total_query.fetch_optional(&state.db).await {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };
  let total = total_row
    .and_then(|row| row.try_get::<Option<i64>, _>("total").ok().flatten())
    .unwrap_or(0);

  let list_sql = format!(
    r#"
    SELECT id, name, description, rule, enabled, created_at, updated_at
    FROM audit_rules
    {where_clause}
    ORDER BY id ASC
    LIMIT ? OFFSET ?
    "#
  );
  let mut list_query = sqlx::query(&list_sql);
  list_query = bind_params(list_query, &params);
  let rows = match list_query
    .bind(limit)
    .bind(offset)
    .fetch_all(&state.db)
    .await
  {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };

  let items = rows
    .into_iter()
    .map(|row| {
      json!({
        "id": row.try_get::<i64, _>("id").unwrap_or(0),
        "name": row.try_get::<Option<String>, _>("name").ok().flatten().unwrap_or_default(),
        "description": row.try_get::<Option<String>, _>("description").ok().flatten().unwrap_or_default(),
        "rule": row.try_get::<Option<String>, _>("rule").ok().flatten().unwrap_or_default(),
        "enabled": row.try_get::<Option<i64>, _>("enabled").unwrap_or(Some(1)).unwrap_or(1),
        "created_at": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("created_at").ok().flatten()),
        "updated_at": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("updated_at").ok().flatten())
      })
    })
    .collect::<Vec<Value>>();

  success(
    json!({
      "data": items,
      "total": total,
      "pagination": {
        "total": total,
        "page": page,
        "limit": limit,
        "pages": if total > 0 { ((total as f64) / (limit as f64)).ceil() as i64 } else { 0 }
      }
    }),
    "Success"
  )
  .into_response()
}

async fn post_audit_rule(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Json(body): Json<AuditRuleRequest>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }

  let name = body.name.unwrap_or_default().trim().to_string();
  let rule = body.rule.unwrap_or_default().trim().to_string();
  if name.is_empty() || rule.is_empty() {
    return error(StatusCode::BAD_REQUEST, "缺少必要字段", None);
  }
  let description = body.description.unwrap_or_default();
  let enabled = body.enabled.unwrap_or(1);

  let result = sqlx::query(
    r#"
    INSERT INTO audit_rules (name, description, rule, enabled, created_at, updated_at)
    VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    "#
  )
  .bind(&name)
  .bind(description)
  .bind(&rule)
  .bind(enabled)
  .execute(&state.db)
  .await;

  let result = match result {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };

  let new_id = result.last_insert_id() as i64;
  let row = sqlx::query("SELECT * FROM audit_rules WHERE id = ?")
    .bind(new_id)
    .fetch_optional(&state.db)
    .await;
  let row = match row {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };

  cache_delete_by_prefix(&state, "audit_rules").await;
  let payload = row.map(map_audit_rule_row).unwrap_or(Value::Null);
  success(payload, "创建成功").into_response()
}

async fn put_audit_rule(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Path(id): Path<i64>,
  Json(body): Json<AuditRuleRequest>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }
  if id <= 0 {
    return error(StatusCode::BAD_REQUEST, "ID 无效", None);
  }

  let mut fields: Vec<String> = Vec::new();
  let mut params: Vec<SqlParam> = Vec::new();

  if let Some(value) = body.name {
    let trimmed = value.trim().to_string();
    if !trimmed.is_empty() {
      fields.push("name = ?".to_string());
      params.push(SqlParam::String(trimmed));
    }
  }
  if let Some(value) = body.description {
    fields.push("description = ?".to_string());
    params.push(SqlParam::String(value));
  }
  if let Some(value) = body.rule {
    let trimmed = value.trim().to_string();
    if !trimmed.is_empty() {
      fields.push("rule = ?".to_string());
      params.push(SqlParam::String(trimmed));
    }
  }
  if let Some(value) = body.enabled {
    fields.push("enabled = ?".to_string());
    params.push(SqlParam::I64(value));
  }

  if fields.is_empty() {
    return error(StatusCode::BAD_REQUEST, "没有需要更新的字段", None);
  }

  let sql = format!(
    "UPDATE audit_rules SET {}, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
    fields.join(", ")
  );
  let mut query_builder = sqlx::query(&sql);
  query_builder = bind_params(query_builder, &params);
  let result = query_builder.bind(id).execute(&state.db).await;
  match result {
    Ok(outcome) => {
      if outcome.rows_affected() == 0 {
        return error(StatusCode::NOT_FOUND, "规则不存在或未更新任何字段", None);
      }
    }
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  }

  let row = sqlx::query("SELECT * FROM audit_rules WHERE id = ?")
    .bind(id)
    .fetch_optional(&state.db)
    .await;
  let row = match row {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };

  cache_delete_by_prefix(&state, "audit_rules").await;
  let payload = row.map(map_audit_rule_row).unwrap_or(Value::Null);
  success(payload, "更新成功").into_response()
}

async fn delete_audit_rule(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Path(id): Path<i64>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }
  if id <= 0 {
    return error(StatusCode::BAD_REQUEST, "ID 无效", None);
  }

  if let Err(err) = sqlx::query("DELETE FROM audit_rules WHERE id = ?")
    .bind(id)
    .execute(&state.db)
    .await
  {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None);
  }

  cache_delete_by_prefix(&state, "audit_rules").await;
  success(Value::Null, "删除成功").into_response()
}

async fn post_audit_rule_test(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Path(id): Path<i64>,
  Json(body): Json<AuditRuleTestRequest>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }
  if id <= 0 {
    return error(StatusCode::BAD_REQUEST, "ID 无效", None);
  }

  let content = body.content.unwrap_or_default();
  let row = sqlx::query("SELECT name, rule FROM audit_rules WHERE id = ?")
    .bind(id)
    .fetch_optional(&state.db)
    .await;
  let row = match row {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };
  let row = match row {
    Some(value) => value,
    None => return error(StatusCode::NOT_FOUND, "审计规则不存在", None)
  };

  let rule = row.try_get::<Option<String>, _>("rule").ok().flatten().unwrap_or_default();
  let name = row.try_get::<Option<String>, _>("name").ok().flatten().unwrap_or_default();
  let matched = !rule.is_empty() && content.contains(&rule);

  success(
    json!({
      "matched": matched,
      "rule_name": name,
      "matched_content": if matched { Value::String(rule) } else { Value::Null }
    }),
    "Success"
  )
  .into_response()
}

async fn get_audit_logs(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Query(query): Query<AuditLogsQuery>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }

  let page = query.page.unwrap_or(1).max(1);
  let limit_raw = query.limit.or(query.page_size).unwrap_or(20);
  let limit = limit_raw.max(1).min(200);
  let offset = (page - 1) * limit;

  let user_search = query.user_search.unwrap_or_default().trim().to_string();
  let rule_id = parse_optional_i64(query.rule_id.as_deref());
  let target_ip = query.target_ip.unwrap_or_default().trim().to_string();
  let start_time = parse_datetime_input(query.start_date.as_deref());
  let end_time = parse_datetime_input(query.end_date.as_deref());

  let mut conditions: Vec<String> = Vec::new();
  let mut params: Vec<SqlParam> = Vec::new();

  if !user_search.is_empty() {
    let parsed_id = user_search.parse::<i64>().unwrap_or(-1);
    let pattern = format!("%{user_search}%");
    conditions.push("(u.email LIKE ? OR u.username LIKE ? OR al.user_id = ?)".to_string());
    params.push(SqlParam::String(pattern.clone()));
    params.push(SqlParam::String(pattern));
    params.push(SqlParam::I64(parsed_id));
  }
  if let Some(value) = rule_id {
    conditions.push("al.audit_rule_id = ?".to_string());
    params.push(SqlParam::I64(value));
  }
  if !target_ip.is_empty() {
    conditions.push("al.ip_address LIKE ?".to_string());
    params.push(SqlParam::String(format!("%{target_ip}%")));
  }
  if let Some(value) = start_time {
    conditions.push("al.created_at >= ?".to_string());
    params.push(SqlParam::String(value));
  }
  if let Some(value) = end_time {
    conditions.push("al.created_at <= ?".to_string());
    params.push(SqlParam::String(value));
  }

  let where_clause = if conditions.is_empty() {
    String::new()
  } else {
    format!("WHERE {}", conditions.join(" AND "))
  };

  let total_sql = format!(
    r#"
    SELECT COUNT(*) as total
    FROM audit_logs al
    LEFT JOIN users u ON al.user_id = u.id
    {where_clause}
    "#
  );
  let mut total_query = sqlx::query(&total_sql);
  total_query = bind_params(total_query, &params);
  let total_row = match total_query.fetch_optional(&state.db).await {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };
  let total = total_row
    .and_then(|row| row.try_get::<Option<i64>, _>("total").ok().flatten())
    .unwrap_or(0);

  let list_sql = format!(
    r#"
    SELECT
      al.id,
      al.user_id,
      u.username,
      u.email AS user_email,
      al.node_id,
      n.name AS node_name,
      al.audit_rule_id,
      ar.name AS rule_name,
      ar.rule AS rule_content,
      al.ip_address,
      al.created_at
    FROM audit_logs al
    LEFT JOIN users u ON al.user_id = u.id
    LEFT JOIN nodes n ON al.node_id = n.id
    LEFT JOIN audit_rules ar ON al.audit_rule_id = ar.id
    {where_clause}
    ORDER BY al.created_at DESC
    LIMIT ? OFFSET ?
    "#
  );
  let mut list_query = sqlx::query(&list_sql);
  list_query = bind_params(list_query, &params);
  let rows = match list_query
    .bind(limit)
    .bind(offset)
    .fetch_all(&state.db)
    .await
  {
    Ok(value) => value,
    Err(err) => return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None)
  };

  let logs = rows
    .into_iter()
    .map(|row| {
      json!({
        "id": row.try_get::<i64, _>("id").unwrap_or(0),
        "user_id": row.try_get::<Option<i64>, _>("user_id").unwrap_or(Some(0)).unwrap_or(0),
        "user_email": row.try_get::<Option<String>, _>("user_email").ok().flatten().unwrap_or_default(),
        "username": row.try_get::<Option<String>, _>("username").ok().flatten().unwrap_or_default(),
        "node_id": row.try_get::<Option<i64>, _>("node_id").unwrap_or(Some(0)).unwrap_or(0),
        "node_name": row.try_get::<Option<String>, _>("node_name").ok().flatten().unwrap_or_default(),
        "audit_rule_id": row.try_get::<Option<i64>, _>("audit_rule_id").unwrap_or(Some(0)).unwrap_or(0),
        "rule_name": row.try_get::<Option<String>, _>("rule_name").ok().flatten().unwrap_or_default(),
        "rule_content": row.try_get::<Option<String>, _>("rule_content").ok().flatten().unwrap_or_default(),
        "ip_address": row.try_get::<Option<String>, _>("ip_address").ok().flatten().unwrap_or_default(),
        "created_at": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("created_at").ok().flatten())
      })
    })
    .collect::<Vec<Value>>();

  success(
    json!({
      "data": logs,
      "total": total,
      "pagination": {
        "total": total,
        "page": page,
        "limit": limit,
        "pages": if total > 0 { ((total as f64) / (limit as f64)).ceil() as i64 } else { 0 }
      }
    }),
    "Success"
  )
  .into_response()
}

async fn delete_audit_log(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Path(id): Path<i64>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }
  if id <= 0 {
    return error(StatusCode::BAD_REQUEST, "ID 无效", None);
  }
  if let Err(err) = sqlx::query("DELETE FROM audit_logs WHERE id = ?")
    .bind(id)
    .execute(&state.db)
    .await
  {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None);
  }
  success(Value::Null, "审计记录已删除").into_response()
}

async fn post_audit_log_batch_delete(
  State(state): State<AppState>,
  Extension(headers): Extension<HeaderMap>,
  Json(body): Json<BatchIdsRequest>
) -> Response {
  if let Err(resp) = require_admin_user_id(&state, &headers, None).await {
    return resp;
  }

  let ids = body.ids.unwrap_or_default();
  let ids = ids.into_iter().filter(|id| *id > 0).collect::<Vec<i64>>();
  if ids.is_empty() {
    return error(StatusCode::BAD_REQUEST, "请提供要删除的记录ID数组", None);
  }

  let placeholders = ids.iter().map(|_| "?").collect::<Vec<&str>>().join(",");
  let sql = format!("DELETE FROM audit_logs WHERE id IN ({placeholders})");
  let mut query_builder = sqlx::query(&sql);
  for id in &ids {
    query_builder = query_builder.bind(id);
  }
  if let Err(err) = query_builder.execute(&state.db).await {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &err.to_string(), None);
  }

  let message = format!("成功删除 {} 条审计记录", ids.len());
  success(json!({ "deleted_count": ids.len() }), &message).into_response()
}

fn parse_optional_i64(value: Option<&str>) -> Option<i64> {
  value
    .map(|value| value.trim())
    .filter(|value| !value.is_empty())
    .and_then(|value| value.parse::<i64>().ok())
}

fn parse_datetime_input(value: Option<&str>) -> Option<String> {
  let raw = value?.trim();
  if raw.is_empty() {
    return None;
  }
  if let Ok(dt) = NaiveDateTime::parse_from_str(raw, "%Y-%m-%d %H:%M:%S") {
    return Some(dt.format("%Y-%m-%d %H:%M:%S").to_string());
  }
  if let Ok(date) = NaiveDate::parse_from_str(raw, "%Y-%m-%d") {
    return date.and_hms_opt(0, 0, 0).map(|dt| dt.format("%Y-%m-%d %H:%M:%S").to_string());
  }
  if let Ok(dt) = DateTime::parse_from_rfc3339(raw) {
    return Some(dt.naive_local().format("%Y-%m-%d %H:%M:%S").to_string());
  }
  None
}

fn format_datetime(value: Option<NaiveDateTime>) -> Option<String> {
  value.map(|dt| dt.format("%Y-%m-%d %H:%M:%S").to_string())
}

fn map_audit_rule_row(row: sqlx::mysql::MySqlRow) -> Value {
  json!({
    "id": row.try_get::<i64, _>("id").unwrap_or(0),
    "name": row.try_get::<Option<String>, _>("name").ok().flatten().unwrap_or_default(),
    "description": row.try_get::<Option<String>, _>("description").ok().flatten().unwrap_or_default(),
    "rule": row.try_get::<Option<String>, _>("rule").ok().flatten().unwrap_or_default(),
    "enabled": row.try_get::<Option<i64>, _>("enabled").unwrap_or(Some(1)).unwrap_or(1),
    "created_at": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("created_at").ok().flatten()),
    "updated_at": format_datetime(row.try_get::<Option<NaiveDateTime>, _>("updated_at").ok().flatten())
  })
}

type SqlxQuery<'a> = sqlx::query::Query<'a, sqlx::MySql, sqlx::mysql::MySqlArguments>;

enum SqlParam {
  I64(i64),
  String(String)
}

fn bind_params<'a>(mut query: SqlxQuery<'a>, params: &'a [SqlParam]) -> SqlxQuery<'a> {
  for param in params {
    query = match param {
      SqlParam::I64(value) => query.bind(*value),
      SqlParam::String(value) => query.bind(value)
    };
  }
  query
}
