...
use crate::cache::{cache_delete, cache_get, cache_get_redis_only, cache_set, cache_set_redis_only};
use crate::crypto::{generate_uuid, hash_password, random_base64, random_numeric_code, random_string, sha256_hex, verify_password};
use crate::templates::email_templates::{
  build_email_subject,
  build_email_text,
  build_verification_html,
  get_verification_title_text
};
...
use crate::referral::{ensure_user_invite_code, find_inviter_by_code, increment_invite_usage, normalize_invite_code, save_referral_relation};
use crate::response::{error, success};
use crate::state::{AppState, PasskeyChallenge, PasskeyChallengeCache, PendingOAuthCache, PendingOAuthRegistration};
...
pub fn router() -> Router<AppState> {
  Router::new()
    ...
    .route("/passkey/register/options", post(post_passkey_register_options))
    .route("/passkey/register/verify", post(post_passkey_register_verify))
...
async fn post_register(
    ...
    match find_inviter_by_code(&state, &invite_code).await {
      Ok(Some(inviter)) => {
        if inviter.invite_limit > 0 && inviter.invite_used >= inviter.invite_limit {
          return error(StatusCode::BAD_REQUEST, "该邀请码使用次数已达上限，请联系邀请人", None);
        }
        inviter_id = Some(inviter.id);
      }
      Ok(None) => {
        return error(StatusCode::BAD_REQUEST, "邀请码无效或已失效，请联系邀请人", None);
      }
      Err(message) => return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None)
    }
  } else if flags.register_mode == "2" {
    return error(StatusCode::FORBIDDEN, "当前仅允许受邀注册，请填写有效邀请码", None);
  }
  ...
  let register_result = register_user(&state, &email, &username, &password, get_client_ip(&headers), inviter_id).await;
  ...
  if let Some(created) = created {
    let response = issue_session(&state, &created, Some("register".to_string()), &headers).await;
...
async fn post_password_reset_confirm(State(state): State<AppState>, Json(body): Json<Value>) -> Response {
  ...
  let new_password = body.get("new_password").and_then(Value::as_str).unwrap_or("").to_string();
...
async fn post_oauth_login(
  ...
  let user = if let Some(user) = user {
    ...
    let register_result = register_user(&state, &normalized_email, &username_value, &temp_password, client_ip.clone(), None).await;
  ...
  let response = issue_session_with_extra(
    ...
    &headers,
    json!({ "provider": provider_key }).as_object().cloned().unwrap_or_default()
  )
  .await;
  match response {
    Ok((token, payload)) => {
...
async fn post_oauth_complete(
  ...
  let pending = match pending {
    Some(value) => value,
    None => return error(StatusCode::GONE, "注册会话已过期，请重新登录并同意条款", None)
  };

  let identifier_field = if pending.provider == "google" { "google_sub" } else { "github_id" };
        ...
        if identifier_field == "google_sub" {
          if let Some(existing) = &user.google_sub {
            if existing != &pending.provider_id {
              return error(StatusCode::CONFLICT, "该邮箱已绑定其他第三方账号，请使用原账号登录", None);
            }
          }
  ...
  if oauth_user.is_none() {
    ...
    if !invite_code.is_empty() {
      ...
      return error(StatusCode::FORBIDDEN, "当前仅允许受邀注册，请输入有效邀请码", None);
    }
    ...
    let provider_label = if pending.provider == "google" { "Google" } else { "GitHub" };
    if let Some(temp_password) = temp_password.as_ref() {
      password_email_sent = send_oauth_welcome_email(
        &state,
        provider_label,
        &pending.email,
        temp_password
  ...
  let user = match oauth_user {
    Some(user) => user,
    None => return error(StatusCode::INTERNAL_SERVER_ERROR, "无法创建或加载用户信息", None)
  };
  if user.status != 1 {
    return error(StatusCode::FORBIDDEN, "账户已禁用", None);
  }

  if let Err(message) = update_oauth_binding(&state, identifier_field, &pending.provider_id, &pending.provider, user.id).await {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None);
  }
  ...
  let refreshed = match refreshed {
    Ok(Some(user)) => user,
    Ok(None) => return error(StatusCode::INTERNAL_SERVER_ERROR, "无法创建或加载用户信息", None),
    Err(message) => return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None)
  };

  let method = if pending.provider == "google" { "google_oauth" } else { "github_oauth" };
...
async fn post_google_oauth(
  ...
  let google_sub = token_info.get("sub").and_then(Value::as_str).unwrap_or("").to_string();
  let email = token_info.get("email").and_then(Value::as_str).unwrap_or("").trim().to_lowercase();
  ...
  if user.is_none() {
    ...
    let pending_token = cache_pending_oauth_registration(
      ...
      PendingOAuthRegistration {
        ...
        fallback_username_seed: if !email_local.is_empty() { email_local.clone() } else { tail_suffix(&google_sub, 6) },
        remember,
        client_ip: get_client_ip(&headers),
        user_agent: headers.get("user-agent").and_then(|v| v.to_str().ok()).map(|v| v.to_string())
      }
    )
    .await;

    let profile_username = candidates
      .first()
      .cloned()
      .unwrap_or_else(|| if !email_local.is_empty() { email_local.clone() } else { format!("google_{}", tail_suffix(&google_sub, 6)) });
  ...
  if let Err(message) = update_oauth_binding(&state, "google_sub", &google_sub, "google", user.id).await {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None);
  }
...
async fn post_github_oauth(
  ...
  let access_token = match exchange_github_token(&client_id, &client_secret, &code, &redirect_uri).await {
    Ok(value) => value,
    Err(message) => return error(StatusCode::UNAUTHORIZED, &message, None)
  };
  ...
  let github_id = gh_user.get("id").and_then(Value::as_i64).map(|v| v.to_string()).unwrap_or_default();
  if github_id.is_empty() {
    return error(StatusCode::BAD_REQUEST, "无效的 GitHub ID", None);
  }

  let mut email = gh_user.get("email").and_then(Value::as_str).unwrap_or("").to_string();
  ...
  if email.trim().is_empty() {
    return error(StatusCode::BAD_REQUEST, "未获取到邮箱，请在 GitHub 公开邮箱后重试", None);
  }
  let normalized_email = email.trim().to_lowercase();
  ...
  if user.is_none() {
    ...
    let pending_token = cache_pending_oauth_registration(
      ...
      PendingOAuthRegistration {
        ...
        username_candidates: candidates.clone(),
        fallback_username_seed: if !email_local.is_empty() { email_local.clone() } else { tail_suffix(&github_id, 6) },
        remember,
        client_ip: get_client_ip(&headers),
        user_agent: headers.get("user-agent").and_then(|v| v.to_str().ok()).map(|v| v.to_string())
      }
    ...
    let profile_username = candidates
      .first()
      .cloned()
      .unwrap_or_else(|| if !email_local.is_empty() { email_local.clone() } else { format!("github_{}", tail_suffix(&github_id, 6)) });
  ...
  if let Err(message) = update_oauth_binding(&state, "github_id", &github_id, "github", user.id).await {
    return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None);
  }
...
async fn post_passkey_register_verify(
  ...
  let received_challenge = extract_client_challenge(&credential.response.client_data_json)
    .unwrap_or_default();
  ...
  let challenge = match challenge {
    Some(value) => value,
    None => return error(StatusCode::BAD_REQUEST, "Passkey 注册会话已过期，请重试", None)
  };
  if challenge.challenge_type != "registration" || challenge.user_id != user.id {
    return error(StatusCode::BAD_REQUEST, "Passkey 注册会话已过期，请重试", None);
  }
  ...
  match result {
    Ok(()) => success(json!({ "credential_id": validated.credential_id }), "Passkey 已绑定").into_response(),
...
async fn post_passkey_login_options(
  ...
  if passkeys.is_empty() {
    return error(StatusCode::BAD_REQUEST, "该账户未绑定 Passkey，请先使用密码登录绑定", None);
  }
...
async fn post_passkey_login_verify(
  ...
  let client_challenge = extract_client_challenge(&credential.response.client_data_json)
    .unwrap_or_default();
  ...
  let final_count = if new_count > passkey.sign_count { new_count } else { passkey.sign_count };
...
async fn post_login(
  ...
  if user.two_factor_enabled == 1 {
    ...
    if !trust_token.is_empty() {
      if let Some(value) = cache_get_redis_only(&state, &format!("2fa_trust_{trust_token}")).await {
    ...
    let challenge_id = create_two_factor_challenge(&state, user.id, body.remember.unwrap_or(false)).await;
    return success(
      json!({
        "need_2fa": true,
    ...
    if let Some(index) = used_index {
      ...
      let _ = update_two_factor_data(&state, user.id, 1, user.two_factor_secret.as_deref(), &backup_codes).await;
    }
  }
...
async fn post_verify_2fa(
  ...
  let trust_token = if body.remember_device.unwrap_or(false) {
    ...
    cache_set_redis_only(&state, &format!("2fa_trust_{token}"), &user.id.to_string(), TRUST_TTL).await;
    Some(token)
  } else {
    None
  };
...
async fn generate_unique_username(
  ...
  for candidate in candidates {
    ...
    if get_user_by_username(state, &normalized).await.ok().flatten().is_none() {
      return normalized;
    }
  }
  ...
  let base = {
    ...
    if normalized.is_empty() { "user".to_string() } else { normalized }
  };
  for _ in 0..6 {
    let next = format!("{}_{}", base, random_string(4));
    if get_user_by_username(state, &next).await.ok().flatten().is_none() {
      return next;
    }
  }
...
async fn send_oauth_welcome_email(
  ...
  match email_service.send_mail(email, &subject, &text, Some(&html)).await {
    Ok(()) => true,
    Err(message) => {
...
async fn fetch_github_email(access_token: &str) -> Result<String, String> {
  ...
  for item in &list {
    let email = item.get("email").and_then(Value::as_str).unwrap_or("").to_string();
    if email.is_empty() {
      continue;
    }
...
fn should_fallback_login_log(err: &sqlx::Error) -> bool {
  match err {
    sqlx::Error::Database(db_err) => {
      let code = db_err.code().map(|value| value == "1054" || value == "1136").unwrap_or(false);
      if code {
        return true;
      }
...
fn is_email_configured(state: &AppState) -> bool {
  let provider = state
    ...
    .to_lowercase();
  if provider == "resend" {
    return state.env.resend_api_key.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false)
      || state.env.mail_resend_key.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false);
  }
  if provider == "smtp" {
    return state.env.mail_smtp_host.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false)
      || state.env.smtp_host.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false);
  }
  if provider == "sendgrid" {
    return state.env.sendgrid_api_key.as_ref().map(|v| !v.trim().is_empty()).unwrap_or(false);
  }
  if provider == "none" {
    return false;
  }
...
async fn handle_verification_code_request(
  ...
  let daily_limit = to_int_config(
    state.env.mail_verification_daily_limit.as_ref(),
    5,
    0,
    true
  );
  ...
  if ip_hourly_limit > 0 && client_ip != "unknown" {
    ...
    if let Ok(Some(row)) = row {
      ...
      if count >= ip_hourly_limit {
        return error(StatusCode::TOO_MANY_REQUESTS, "请求过于频繁，请稍后再试或更换网络", None);
      }
    }
  }

  let (code, expires_at) = match send_email_code(state, &email, purpose, &client_ip, &user_agent).await {
    Ok(value) => value,
    Err(message) => return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None)
  };

  let log_prefix = if purpose == "password_reset" { "password-reset" } else { "email-code" };
  println!("[{}] purpose={} email={} code={}", log_prefix, purpose, email, code);
  success(json!({ "expires_at": expires_at }), "验证码已发送，请查收邮箱").into_response()
}
  ...
  if hash != code_hash {
    ...
    let _ = sqlx::query(
      &format!(
        ...
        if reach_limit { "CURRENT_TIMESTAMP" } else { "used_at" }
      )
    )
    .bind(next_attempts)
    .bind(id)
...
async fn reset_password_with_code(
  ...
  verify_email_code(state, email, "password_reset", code, get_verification_attempt_limit(state)).await?;
...
async fn register_user(
  ...
  let account_expire_days = to_int_config(configs.get("default_account_expire_days"), 3650, 0, true);
  ...
  let account_expire_time = if account_expire_days > 0 {
    Some((now + chrono::Duration::days(account_expire_days)).format("%F %T").to_string())
  } else {
    None
  };
  let class_expire_time = if class_expire_days > 0 {
    Some((now + chrono::Duration::days(class_expire_days)).format("%F %T").to_string())
  } else {
    None
  };
...
async fn resolve_verification_flags(state: &AppState) -> Result<VerificationFlags, String> {
  ...
  let register_mode = configs.get("register_enabled").cloned().unwrap_or_else(|| "1".to_string());
  let register_enabled = register_mode != "0";
  let email_verify_enabled = configs
    .get("register_email_verification_enabled")
    .map(|value| value != "0")
...
fn to_int_config(value: Option<&String>, fallback: i64, min: i64, allow_zero: bool) -> i64 {
  let parsed = value.and_then(|value| value.parse::<i64>().ok()).unwrap_or(fallback);
  if parsed < min {
    return fallback;
  }
...
pub(super) fn decrypt_two_factor_secret(state: &AppState, encrypted: Option<&str>) -> Result<String, String> {
...
fn two_factor_key(state: &AppState) -> Vec<u8> {
  ...
  let secret = if !env_key.is_empty() {
    ...
    let jwt = state.env.jwt_secret.as_deref().unwrap_or("").trim().to_string();
    if !jwt.is_empty() {
      jwt
    } else {
...
impl PasskeyRow {
  fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
    Self {
      ...
      user_handle: row.try_get::<Option<String>, _>("user_handle").ok().flatten(),
      rp_id: row.try_get::<Option<String>, _>("rp_id").ok().flatten(),
      transports: row.try_get::<Option<String>, _>("transports").ok().flatten(),
      sign_count: row.try_get::<Option<i64>, _>("sign_count").unwrap_or(Some(0)).unwrap_or(0),
      device_name: row.try_get::<Option<String>, _>("device_name").ok().flatten(),
      last_used_at: row
...
pub(super) async fn list_passkeys(state: &AppState, user_id: i64) -> Result<Vec<PasskeyRow>, String> {
...
async fn verify_turnstile(
  ...
  let client = reqwest::Client::new();
  let mut params = vec![("secret", secret.to_string()), ("response", token.to_string())];
...
pub(super) async fn require_user_id(
  ...
  require_user(state, headers, token_override).await.map(|user| user.id)
}
  ...
  fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
    Self {
      ...
      two_factor_backup_codes: row
        ...
        .ok()
        .flatten(),
      google_sub: row.try_get::<Option<String>, _>("google_sub").ok().flatten(),
