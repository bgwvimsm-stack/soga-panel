...
async fn get_withdrawals(
    ...
    let total_row =
        sqlx::query("SELECT COUNT(*) as total FROM rebate_withdrawals WHERE user_id = ?")
            .bind(user_id)
            .fetch_optional(&state.db)
            .await;
...
async fn get_transactions(
    ...
    let total_row =
        sqlx::query("SELECT COUNT(*) as total FROM rebate_transactions WHERE inviter_id = ?")
            .bind(user_id)
            .fetch_optional(&state.db)
            .await;
    let total_row = match total_row {
...
async fn post_transfer(
    ...
    let _ = insert_user_transaction(
        &state, user_id, -amount, "transfer", "balance", None, None, None,
    )
    .await;
