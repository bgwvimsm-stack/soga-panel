...
async fn post_recharge(
    ...
    if let Err(message) =
        create_recharge_record(&state, user_id, amount, &trade_no, selected_channel).await
    {
...
async fn post_gift_card_redeem(
    ...
    match card.card_type.as_str() {
        "balance" => {
            ...
            if let Err(message) =
                create_recharge_record(&state, user_id, amount, &trade_no, "gift_card").await
            {
                return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None);
            }
      ...
      .await;
            ...
            if let Some(expire) = apply.new_expire_time.as_ref() {
                let update =
                    sqlx::query("UPDATE package_purchase_records SET expires_at = ? WHERE id = ?")
                        .bind(expire)
                        .bind(record_id)
                        .execute(&state.db)
                        .await;
    ...
    if let Err(message) =
        mark_gift_card_used(&state, card.id, card.used_count, card.max_usage).await
    {
        return error(StatusCode::INTERNAL_SERVER_ERROR, &message, None);
    }
...
async fn get_user_balance(state: &AppState, user_id: i64) -> Result<f64, String> {
    ...
    Ok(row
        .map(|value| parse_decimal(&value, "money", 0.0))
        .unwrap_or(0.0))
}
    ...
    if let Some(forwarded_host) = header("x-forwarded-host") {
        if !forwarded_host.is_empty() {
            ...
            return format!(
                ...
                forwarded_host.split(',').next().unwrap_or("").trim()
            );
        }
    }
...
async fn get_gift_card_by_code(
    state: &AppState,
    code: &str,
) -> Result<Option<GiftCardRow>, String> {
    let row = sqlx::query(
    r#"
  ...
  .map_err(|err| err.to_string())?;
    ...
    Ok(row.map(|row| GiftCardRow {
        id: row.try_get::<i64, _>("id").unwrap_or(0),
        card_type: row
            .try_get::<Option<String>, _>("card_type")
            .ok()
            .flatten()
            .unwrap_or_default(),
        status: row
            ...
            .unwrap_or(0),
        balance_amount: row
            .try_get::<Option<f64>, _>("balance_amount")
            .ok()
            .flatten(),
        duration_days: row
            ...
            .ok()
            .flatten(),
        traffic_value_gb: row
            .try_get::<Option<i64>, _>("traffic_value_gb")
            .ok()
            .flatten(),
        reset_traffic_gb: row
        ...
        used_count: row
            ...
            .unwrap_or(0),
        start_at: row
            .try_get::<Option<NaiveDateTime>, _>("start_at")
            .ok()
            .flatten(),
        end_at: row
...
async fn get_package_by_id_any(
    state: &AppState,
    package_id: i64,
    ...
    Ok(row.map(|row| PackageRow {
        ...
        traffic_quota: row
            .try_get::<Option<i64>, _>("traffic_quota")
            .ok()
            .flatten(),
        validity_days: row
            .try_get::<Option<i64>, _>("validity_days")
            .ok()
            .flatten(),
        level: row
...
async fn update_user_after_package_purchase(
    ...
    let current_transfer_enable = user_row
        ...
        .flatten()
        .unwrap_or(0);
    let package_traffic_bytes =
        package.traffic_quota.unwrap_or(0) as f64 * 1024.0 * 1024.0 * 1024.0;
