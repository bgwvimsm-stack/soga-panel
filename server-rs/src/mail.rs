...
impl EmailService {
    ...
    fn get_smtp_host(&self) -> Option<String> {
        self.read_env("MAIL_SMTP_HOST")
            .or_else(|| self.read_env("SMTP_HOST"))
    }

    fn get_smtp_port(&self) -> Option<u16> {
        self.env.mail_smtp_port.or(self.env.smtp_port)
    }

    fn get_smtp_user(&self) -> Option<String> {
        self.read_env("MAIL_SMTP_USER")
            .or_else(|| self.read_env("SMTP_USER"))
    }

    fn get_smtp_pass(&self) -> Option<String> {
        self.read_env("MAIL_SMTP_PASS")
            .or_else(|| self.read_env("SMTP_PASS"))
    }
    ...
    async fn send_via_resend(
        ...
        let api_key = self
            .read_env("RESEND_API_KEY")
            .or_else(|| self.read_env("MAIL_RESEND_KEY"));
        let api_key = api_key.ok_or_else(|| {
            "未配置 RESEND_API_KEY（或 MAIL_RESEND_KEY），无法发送邮件".to_string()
        })?;
    ...
    async fn send_via_smtp(
        ...
        let secure = matches!(
            self.read_env("SMTP_SECURE").as_deref(),
            Some("true") | Some("1")
        );
        let port = self
            .get_smtp_port()
        ...
        let credentials =
            user.and_then(|user| pass.clone().map(|pass| Credentials::new(user, pass)));
    ...
    fn assert_resend_from_domain(&self, from: &str) -> Result<(), String> {
        let email = extract_email_address(from)
            .ok_or_else(|| "MAIL_FROM 格式不正确，需为 email 或 `Name <email>`".to_string())?;
